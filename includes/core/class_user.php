...
class User
{
    ...
    public static function users_list($d = [])
    {
        ...
        while ($row = DB::fetch_row($q)) {
            $items[] = [
                'id' => (int)$row['user_id'],
                'plot_id' => self::explode_trim_arr($row['plot_id']),
    ...
    public static function explode_trim($plots)
    {
        $plots = explode(',', $plots);
        $new_plots = '';
        foreach ($plots as $plot) {
            if (trim($plot)) $new_plots .= trim($plot) . ', ';
        }
        $new_plots = substr($new_plots, 0, -2);
        return $new_plots;
    }

    public static function explode_trim_arr($plots)
    {
        $plots = explode(',', $plots);
        $new_plots = [];
        for ($i = 0, $c_p = count($plots); $i < $c_p; $i++) {
            if (trim($plots[$i])) $plot = trim($plots[$i]);
            if ($i + 1 < $c_p) $plot .= ', ';
            $new_plots[] = $plot;
        }
        return $new_plots;
